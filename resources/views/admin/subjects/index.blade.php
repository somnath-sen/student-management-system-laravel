...
<div class="max-w-7xl mx-auto">
    ...
    <div id="bulk-actions" class="hidden mb-4 p-4 bg-indigo-50 border border-indigo-100 rounded-xl flex items-center justify-between animate-enter stagger-1">
        <div class="flex items-center gap-3">
            ...
            <span class="text-indigo-900 font-semibold text-sm">Subjects Selected</span>
        </div>
        <div class="flex items-center gap-3">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($subjects as $subject)
                        <tr class="table-row group">
                            <td class="px-6 py-4">
                                <input type="checkbox" class="subject-checkbox w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 cursor-pointer" value="{{ $subject->id }}">
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.subjects.destroy', $subject->id) }}" data-confirm="⚠️ Are you sure you want to delete this subject? This will affect results associated with it.">
                                        @csrf
                                        @method('DELETE')
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        checkboxes.forEach(cb => {
            cb.addEventListener('change', function () {
                ...
                } else if (document.querySelectorAll('.subject-checkbox:checked').length === checkboxes.length) {
        ...
        window.confirmBulkDelete = function () {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: `You are about to delete ${selectedIds.length} selected subject(s). This action cannot be undone.`,
                icon: 'warning',
                ...
                preConfirm: () => {
                    return fetch('{{ route('admin.subjects.bulk-delete') }}', {
                        method: 'POST',
                        headers: {
