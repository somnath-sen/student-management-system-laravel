...
@section('title', 'Faculty Applications')
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Faculty Applications</h1>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-[#FDFBF7] border-b border-[#F0EBE1]">
                        <th class="py-4 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider">Date Applied</th>
                        <th class="py-4 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider">Applicant Info</th>
                        <th class="py-4 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider">Subject Expertise</th>
                        <th class="py-4 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider">Verification Docs</th>
                ...
                <tbody class="divide-y divide-[#F0EBE1]">
                    @forelse($applicants as $app)
                        @php
                            // MAGIC STEP: Check if this email already exists in the Laravel database
                        ...
                        <tr class="hover:bg-slate-50 transition-colors {{ $isApproved ? 'opacity-75 bg-slate-50/50' : '' }}">
                            ...
                            <td class="py-4 px-6">
                                <div class="font-bold text-slate-900 text-base">{{ $app['Name'] ?? 'N/A' }}</div>
                                <div class="text-sm text-slate-500 mt-0.5">{{ $app['Email'] ?? 'N/A' }}</div>
                                <div class="text-xs text-purple-600 font-bold mt-1.5 flex items-center gap-1.5">
                                    <i class="fa-solid fa-phone"></i> {{ $app['Phone'] ?? 'N/A' }}
                                </div>
                            </td>
                            
                            <td class="py-4 px-6">
                                <span class="px-3 py-1.5 bg-purple-50 text-purple-700 rounded-lg text-xs font-bold border border-purple-100 shadow-sm inline-block">
                                    {{ $app['Subject'] ?? 'N/A' }}
                                </span>
                            </td>
                            ...
                            <td class="py-4 px-6">
                                <div class="flex gap-2">
                                    <a href="{{ $app['Photo'] ?? '#' }}" target="_blank" class="w-9 h-9 rounded-xl bg-[#FDFBF7] border border-[#F0EBE1] text-slate-500 flex items-center justify-center hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all shadow-sm group" title="View Headshot">
                            ...
                            <td class="py-4 px-6 text-right">
                                ...
                                @else
                                    <a href="{{ route('admin.teachers.create', ['name' => $app['Name'] ?? '', 'email' => $app['Email'] ?? '']) }}" 
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-16 text-center">
                                ...
                                <h3 class="text-lg font-bold text-slate-700 mb-1">No Faculty Applications</h3>
