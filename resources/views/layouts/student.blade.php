    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; }
        ...
        .hdr-logout {
            ...
            color: #FF3B30; font-size: 0.78rem; font-weight: 600;
        ...
        .bottom-tab {
            ...
            color: var(--text-muted); font-size: 0.56rem; font-weight: 500;
        ...
        /* ── ANIMATIONS ───────────────────────────────────────── */
        @keyframes ios-in { from { opacity:0; transform: translateY(12px) scale(0.98); } to { opacity:1; transform: translateY(0) scale(1); } }
        .ios-in { animation: ios-in 0.35s cubic-bezier(0.25,0.46,0.45,0.94) forwards; }
        @keyframes ios-in-fast { from { opacity:0; transform: translateY(6px); } to { opacity:1; transform: translateY(0); } }
        .ios-in-fast { animation: ios-in-fast 0.22s ease-out forwards; }
    </style>
</head>
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('student.report-card.download') }}" id="sidebar-report-card-link" class="nav-item" style="color:#FF9F0A;">
                <span class="nav-icon" style="background:rgba(255,159,10,0.15);color:#FF9F0A;"><i class="fa-solid fa-file-pdf"></i></span>
            ...
            <a href="{{ route('student.location') }}" class="nav-item {{ request()->routeIs('student.location.*') ? 'active' : '' }}">
                ...
                <span class="nav-badge" style="background:rgba(255,59,48,0.10);color:#FF3B30;display:flex;align-items:center;gap:4px;">
        ...
        <div class="sidebar-footer">
            <div class="user-chip">
                ...
                <div style="overflow:hidden;flex:1;">
                    <div style="font-size:0.82rem;font-weight:600;color:var(--text-primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ auth()->user()->name ?? 'Student' }}</div>
            ...
            <div style="display:flex;align-items:center;gap:10px;">
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                    ...
                    <div x-show="open" style="display:none;"
                        ...
                        <div style="padding:16px 18px 12px;border-bottom:1px solid var(--divider);display:flex;justify-content:space-between;align-items:center;">
                            <span style="font-size:0.9rem;font-weight:700;color:var(--text-primary);">Notifications</span>
                            @if(isset($latestNotices) && $latestNotices->count() > 0)
                                <span style="font-size:0.65rem;font-weight:700;color:var(--accent);padding:3px 9px;border-radius:100px;background:var(--accent-soft);">{{ $latestNotices->count() }} New</span>
                            @endif
                        </div>
                        ...
                        <div style="max-height:320px;overflow-y:auto;padding:8px;">
                            @if(isset($latestNotices) && $latestNotices->count() > 0)
                                ...
                                @php $nc=['Urgent'=>'#FF3B30','Exam'=>'var(--accent)','Holiday'=>'#34C759','General'=>'#007AFF'];$cl=$nc[$notice->category]??$nc['General']; @endphp
                                <a href="{{ route('student.notices.show', $notice) }}" class="notif-row" style="display:block;padding:10px 10px;text-decoration:none;border-radius:12px;margin-bottom:2px;">
                                    <div style="display:flex;gap:10px;align-items:flex-start;">
                                        <div style="width:34px;height:34px;border-radius:10px;background:var(--border-2);display:flex;align-items:center;justify-content:center;color:{{ $cl }};flex-shrink:0;">
                                            <i class="fa-solid fa-bullhorn" style="font-size:0.75rem;"></i>
                                        </div>
                                        <div style="flex:1;min-width:0;">
                                            <p style="font-size:0.8rem;font-weight:600;color:var(--text-primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $notice->title }}</p>
                                            <p style="font-size:0.7rem;color:var(--text-muted);margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ Str::limit($notice->content, 42) }}</p>
                                            <p style="font-size:0.58rem;color:var(--text-muted);margin-top:3px;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;">{{ $notice->created_at->diffForHumans() }}</p>
                                        </div>
                                    </div>
                                </a>
                                @endforeach
                                <a href="{{ route('student.dashboard') }}" style="display:block;text-align:center;padding:11px;font-size:0.75rem;font-weight:700;color:var(--accent);text-decoration:none;border-radius:12px;" class="notif-row">
                                    View All →
                                </a>
                            @else
                                <div style="padding:28px 20px;text-align:center;">
                                    <div style="width:40px;height:40px;border-radius:12px;background:var(--border-2);display:flex;align-items:center;justify-content:center;margin:0 auto 10px;color:var(--text-muted);">
                                        <i class="fa-solid fa-check"></i>
                                    </div>
                                    <p style="font-size:0.82rem;font-weight:600;color:var(--text-secondary);">All caught up!</p>
                                </div>
                            @endif
                        </div>
                    </div>
...
<div x-data="{ moreOpen: false }" class="lg:hidden">
        ...
        <div style="display:flex;justify-content:space-around;align-items:center;padding:6px 8px 10px;max-width:480px;margin:0 auto;">
            ...
            <a href="{{ route('studyai.index') }}" class="bottom-tab {{ request()->routeIs('studyai.*') ? 'is-active' : '' }}">
                <div style="position:relative;">
                    ...
                    <span style="position:absolute;top:-3px;right:-6px;width:6px;height:6px;background:#AF52DE;border-radius:50%;border:1.5px solid var(--bg);" class="animate-pulse"></span>
                </div>
                <span>StudyAI</span>
            </a>
    ...
    <div x-show="moreOpen" style="display:none;"
         x-transition:enter="transition-opacity duration-250" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
    ...
    <div x-show="moreOpen" style="display:none;"
         ...
         class="more-drawer fixed bottom-0 inset-x-0 z-[80] flex flex-col" style="max-height:88vh;">
        ...
        <div style="padding:14px 18px 10px;border-bottom:1px solid var(--divider);display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
            ...
            <button @click="moreOpen = false" class="hdr-btn" style="width:30px;height:30px;border-radius:8px;margin-top:7px;">
                <i class="fa-solid fa-xmark" style="font-size:0.85rem;"></i>
            </button>
        </div>

        <div style="overflow-y:auto;flex:1;padding:12px 12px 28px;">
            <p style="font-size:0.62rem;font-weight:600;letter-spacing:0.06em;text-transform:uppercase;color:var(--text-muted);padding:4px 4px 8px;">Academic</p>
            <a href="{{ route('student.timetable') }}"       class="drawer-row"><span class="drawer-icon" style="color:#007AFF;"><i class="fa-solid fa-clock"></i></span>Class Routine</a>
            <a href="{{ route('student.exams.index') }}"     class="drawer-row"><span class="drawer-icon" style="color:#5856D6;"><i class="fa-solid fa-file-pen"></i></span>Exam Schedule</a>
            <a href="{{ route('student.admit-card.show') }}" class="drawer-row"><span class="drawer-icon" style="color:#32ADE6;"><i class="fa-solid fa-id-card-clip"></i></span>Admit Card</a>
            <a href="{{ route('student.results.index') }}"   class="drawer-row"><span class="drawer-icon" style="color:#FF9F0A;"><i class="fa-solid fa-trophy"></i></span>Results</a>
            <a href="{{ route('student.marksheet.show') }}"  class="drawer-row"><span class="drawer-icon" style="color:#FF6B00;"><i class="fa-solid fa-file-lines"></i></span>Marksheet</a>
            @if($reportCardPublished)
            <a href="{{ route('student.report-card.download') }}" class="drawer-row"><span class="drawer-icon" style="color:#FF9F0A;"><i class="fa-solid fa-file-pdf"></i></span><span style="flex:1">Report Card</span><span class="nav-badge" style="background:rgba(255,159,10,0.15);color:#FF9F0A;">PDF</span></a>
            @endif

            <p style="font-size:0.62rem;font-weight:600;letter-spacing:0.06em;text-transform:uppercase;color:var(--text-muted);padding:14px 4px 8px;">AI & Analytics</p>
            <a href="{{ route('student.attendance.insights') }}" class="drawer-row"><span class="drawer-icon" style="color:#AF52DE;"><i class="fa-solid fa-lightbulb"></i></span><span style="flex:1">Attendance Insights</span><span class="nav-badge" style="background:rgba(175,82,222,0.12);color:#AF52DE;">AI</span></a>
            <a href="{{ route('student.suggestions') }}"        class="drawer-row"><span class="drawer-icon" style="color:#AF52DE;"><i class="fa-solid fa-wand-magic-sparkles"></i></span>AI Suggestions</a>

            <p style="font-size:0.62rem;font-weight:600;letter-spacing:0.06em;text-transform:uppercase;color:var(--text-muted);padding:14px 4px 8px;">Campus</p>
            <a href="{{ route('student.dashboard') }}#announcements" class="drawer-row"><span class="drawer-icon" style="color:#5856D6;"><i class="fa-solid fa-bullhorn"></i></span>Broadcasts</a>
            <a href="{{ route('student.smart-id') }}"    class="drawer-row"><span class="drawer-icon" style="color:#34C759;"><i class="fa-solid fa-id-badge"></i></span>Smart ID Card</a>
            <a href="{{ route('student.location') }}"    class="drawer-row"><span class="drawer-icon" style="color:#FF3B30;"><i class="fa-solid fa-location-crosshairs"></i></span><span style="flex:1">Family Tracker</span><span style="width:6px;height:6px;border-radius:50%;background:#FF3B30;" class="animate-pulse"></span></a>
            <a href="{{ route('student.emergency') }}"   class="drawer-row"><span class="drawer-icon" style="color:#FF3B30;"><i class="fa-solid fa-shield-halved"></i></span>Emergency Info</a>
            ...
            <a href="{{ route('student.fees.index') }}"  class="drawer-row"><span class="drawer-icon" style="color:#32ADE6;"><i class="fa-solid fa-credit-card"></i></span>My Fees</a>
