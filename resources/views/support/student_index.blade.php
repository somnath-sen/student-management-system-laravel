...
<style>
/* ════════════════════════════════════════════════════════════
   STUDENT SUPPORT INDEX  — No popup, fully self-contained
════════════════════════════════════════════════════════════ */
...
/* ── Hero ── */
.sp-hero {
    background: linear-gradient(135deg, #007AFF 0%, #5AC8FA 60%, #34AADC 100%);
    border-radius: 24px; padding: 32px 32px 28px; margin-bottom: 28px;
...
.sp-ask-btn {
    ...
    color:#fff;font-size:0.88rem;font-weight:700;cursor:pointer;transition:all 0.22s ease;
...
/* ── Inline Ask Doubt Form ── */
.sp-ask-form {
    background:#fff;border-radius:22px;border:1.5px solid rgba(0,122,255,0.2);
...
.sp-form-title { font-size:1rem;font-weight:800;color:#1C1C1E;letter-spacing:-0.025em;margin-bottom:20px;display:flex;align-items:center;gap:10px; }
.sp-form-field { margin-bottom:16px; }
.sp-form-label { font-size:0.72rem;font-weight:600;color:rgba(60,60,67,0.6);margin-bottom:7px;display:block;text-transform:uppercase;letter-spacing:0.06em; }
.sp-form-input, .sp-form-textarea {
    width:100%;border:1.5px solid rgba(60,60,67,0.15);border-radius:14px;
    padding:13px 16px;font-size:0.88rem;font-family:inherit;
    background:#F9F9FB;color:#1C1C1E;outline:none;transition:all 0.18s;
}
.sp-form-input:focus, .sp-form-textarea:focus { border-color:#007AFF;background:#fff;box-shadow:0 0 0 3px rgba(0,122,255,0.1); }
.sp-form-textarea { resize:vertical;min-height:110px; }
.sp-form-row { display:flex;gap:10px;margin-top:4px;flex-wrap:wrap; }
.sp-form-submit {
    flex:1;min-width:140px;padding:13px 24px;border-radius:14px;
    ...
    transition:all 0.2s;box-shadow:0 4px 14px rgba(0,122,255,0.3);
}
.sp-form-submit:hover { transform:translateY(-2px);box-shadow:0 8px 24px rgba(0,122,255,0.4); }
...
.sp-form-cancel:hover { background:rgba(60,60,67,0.1); }
.sp-form-success { display:none;text-align:center;padding:24px;background:rgba(52,199,89,0.06);border-radius:16px;border:1px solid rgba(52,199,89,0.2); }
...
.sp-stat { background:#fff;border:1px solid rgba(60,60,67,0.1);border-radius:18px;padding:18px 16px;text-align:center;transition:all 0.2s; }
...
.sp-section-title { font-size:1rem;font-weight:800;color:#1C1C1E;letter-spacing:-0.025em; }

/* ── Ticket Card ── */
.sp-ticket { background:#fff;border:1.5px solid rgba(60,60,67,0.1);border-radius:22px;overflow:hidden;transition:all 0.25s cubic-bezier(0.34,1.56,0.64,1);margin-bottom:14px; }
.sp-ticket:hover { transform:translateY(-2px);box-shadow:0 10px 32px rgba(0,0,0,0.09); }
.sp-ticket.is-solved { border-color:rgba(52,199,89,0.25); }
.sp-ticket.is-in_progress { border-color:rgba(0,122,255,0.25); }
...
.sp-ticket-subject { font-size:0.95rem;font-weight:800;color:#1C1C1E;letter-spacing:-0.02em; }
...
.sp-pill-in_progress{ background:rgba(255,159,10,0.12);color:#FF9F0A; }
...
.sp-step-dot { width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:0.75rem;font-weight:700;transition:all 0.4s cubic-bezier(0.34,1.56,0.64,1);position:relative; }
.sp-step-dot.done    { background:linear-gradient(135deg,#30D158,#34C759);color:#fff;box-shadow:0 4px 14px rgba(52,199,89,0.4); }
.sp-step-dot.active  { background:linear-gradient(135deg,#007AFF,#5AC8FA);color:#fff;box-shadow:0 4px 14px rgba(0,122,255,0.4);animation:spStepPulse 2s infinite; }
.sp-step-dot.pending { background:rgba(60,60,67,0.08);color:rgba(60,60,67,0.3);border:1.5px solid rgba(60,60,67,0.12); }
@keyframes spStepPulse { 0%,100%{box-shadow:0 4px 14px rgba(0,122,255,0.4);transform:scale(1);}50%{box-shadow:0 4px 24px rgba(0,122,255,0.7);transform:scale(1.08);} }
...
.sp-step-label.active-lbl { color:#007AFF; }
...
.sp-connector.active  { background:linear-gradient(90deg,#30D158,rgba(0,122,255,0.4)); }
.sp-connector.pending { background:rgba(60,60,67,0.1); }

/* ── Admin Reply Bubble ── */
.sp-reply-wrap { border-top:1px solid rgba(60,60,67,0.07);padding:16px 22px;background:linear-gradient(135deg,rgba(0,122,255,0.04),rgba(90,200,250,0.04)); }
.sp-reply-label { font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#007AFF;margin-bottom:10px;display:flex;align-items:center;gap:6px; }
.sp-reply-bubble { background:#fff;border:1.5px solid rgba(0,122,255,0.18);border-radius:6px 18px 18px 18px;padding:14px 18px;font-size:0.85rem;color:#1C1C1E;line-height:1.65;box-shadow:0 4px 14px rgba(0,122,255,0.08); }
...
.sp-pulse.submitted   { background:#007AFF;animation:spPulseAnim 2s infinite; }
.sp-pulse.in_progress { background:#FF9F0A;animation:spPulseAnim 1.4s infinite; }
.sp-pulse.solved      { background:#30D158; }
@keyframes spPulseAnim { 0%,100%{opacity:1;transform:scale(1);}50%{opacity:0.5;transform:scale(0.85);} }

/* ── Empty State ── */
.sp-empty { text-align:center;padding:64px 20px;background:#fff;border-radius:24px;border:1.5px dashed rgba(60,60,67,0.15); }
.sp-empty-icon { width:72px;height:72px;border-radius:22px;background:linear-gradient(135deg,rgba(0,122,255,0.1),rgba(90,200,250,0.1));display:flex;align-items:center;justify-content:center;margin:0 auto 18px;font-size:1.8rem;color:#007AFF; }
.sp-live-badge { display:flex;align-items:center;gap:6px;font-size:0.65rem;font-weight:600;color:#007AFF;padding:4px 12px;background:rgba(0,122,255,0.08);border-radius:100px; }
.sp-live-dot { width:6px;height:6px;background:#007AFF;border-radius:50%;animation:spPA 1.5s infinite; }
...
.sp-chat-admin-av { width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,#007AFF,#5AC8FA);display:flex;align-items:center;justify-content:center;color:#fff;font-size:0.65rem;flex-shrink:0; }
.sp-chat-admin-bubble { background:#fff;border:1.5px solid rgba(0,122,255,0.15);border-radius:16px 16px 16px 4px;padding:12px 16px;max-width:76%;font-size:0.84rem;line-height:1.65;color:#1C1C1E;box-shadow:0 3px 10px rgba(0,122,255,0.08); }
.sp-chat-admin-lbl { font-size:0.6rem;font-weight:700;color:#007AFF;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.06em; }
.sp-chat-user-wrap { display:flex;align-items:flex-end;justify-content:flex-end;gap:8px; }
.sp-chat-user-bubble { background:linear-gradient(135deg,#007AFF,#5AC8FA);color:#fff;border-radius:16px 16px 4px 16px;padding:12px 16px;max-width:76%;font-size:0.84rem;line-height:1.65;box-shadow:0 3px 12px rgba(0,122,255,0.22); }
...
/* ── User Reply Bar ── */
.sp-user-reply-bar { border-top:1px solid rgba(60,60,67,0.07);padding:14px 22px 18px;background:rgba(0,122,255,0.02); }
.sp-user-reply-label { font-size:0.7rem;font-weight:700;color:#007AFF;margin-bottom:8px;display:flex;align-items:center;gap:6px;text-transform:uppercase;letter-spacing:0.06em; }
.sp-user-reply-row { display:flex;gap:10px;align-items:flex-end; }
.sp-user-reply-textarea { flex:1;border:1.5px solid rgba(0,122,255,0.2);border-radius:14px;padding:11px 14px;font-size:0.85rem;font-family:inherit;background:#fff;color:#1C1C1E;resize:none;min-height:62px;max-height:140px;outline:none;transition:border-color 0.18s;line-height:1.6; }
.sp-user-reply-textarea:focus { border-color:#007AFF;box-shadow:0 0 0 3px rgba(0,122,255,0.1); }
.sp-user-reply-btn { padding:11px 20px;border-radius:14px;background:linear-gradient(135deg,#007AFF,#5AC8FA);color:#fff;font-size:0.82rem;font-weight:700;border:none;cursor:pointer;display:inline-flex;align-items:center;gap:7px;flex-shrink:0;box-shadow:0 4px 12px rgba(0,122,255,0.3);transition:all 0.18s; }
...
@media(max-width:640px){ .sp-stats{grid-template-columns:repeat(3,1fr);gap:8px;} .sp-hero{padding:24px 20px;} .sp-hero-title{font-size:1.25rem;} }
</style>
...
<div class="sp-wrap">
    ...
    <div class="sp-hero">
        <div class="sp-hero-title">
            <i class="fa-solid fa-headset" style="margin-right:10px;opacity:0.85;"></i>My Support & Doubts
        </div>
        <div class="sp-hero-sub">Submit questions and track their status. Chat with our support team in real-time.</div>
        <button class="sp-ask-btn" onclick="openSupportModal()" id="askDoubtBtn">
    ...
    <div class="sp-stats">
        <div class="sp-stat">
            <div class="sp-stat-num" style="color:#1C1C1E;">{{ $totalCount }}</div>
            <div class="sp-stat-lbl">Total</div>
        </div>
        <div class="sp-stat">
            <div class="sp-stat-num" style="color:#FF9F0A;">{{ $pendingCount + $ipCount }}</div>
            <div class="sp-stat-lbl">Open</div>
        </div>
        <div class="sp-stat">
            <div class="sp-stat-num" style="color:#30D158;">{{ $doneCount }}</div>
            <div class="sp-stat-lbl">Solved</div>
        </div>
    </div>
    ...
    @if($tickets->isEmpty())
        <div class="sp-empty">
            ...
            <p style="font-size:1.05rem;font-weight:800;color:#1C1C1E;letter-spacing:-0.025em;margin-bottom:8px;">No doubts yet</p>
            <p style="font-size:0.82rem;color:rgba(60,60,67,0.55);margin-bottom:24px;line-height:1.6;">Ask your first question and our support team will help you quickly!</p>
            <button class="sp-ask-btn" onclick="openSupportModal()" style="background:linear-gradient(135deg,#007AFF,#5AC8FA);border:none;padding:14px 28px;font-size:0.9rem;">
    ...
    @else
        <div id="ticketContainer">
            ...
            <div class="sp-ticket is-{{ $ticket->status }}" id="sp-ticket-{{ $ticket->id }}" data-status="{{ $ticket->status }}">
                ...
                {{-- ── Chat Thread: all messages from support_messages table ── --}}
                ...
                @if($chatEnabled && $ticket->status !== 'solved')
                    <div class="sp-user-reply-bar" id="sp-reply-bar-{{ $ticket->id }}">
                        <div class="sp-user-reply-label">
                            <i class="fa-solid fa-reply"></i> Reply to Support
                        </div>
                        <div class="sp-user-reply-row">
                            <textarea
                                class="sp-user-reply-textarea"
                                id="sp-reply-text-{{ $ticket->id }}"
                                placeholder="Type your follow-up message… Press Ctrl+Enter to send"
                                rows="3"></textarea>
                            <button
                                class="sp-user-reply-btn"
                                id="sp-reply-btn-{{ $ticket->id }}"
                                onclick="userSendMessage({{ $ticket->id }})"
                            >
                ...
                @if($ticket->status === 'solved')
                    <div class="sp-solved-banner">
                        <i class="fa-solid fa-circle-check" style="font-size:1rem;"></i>
                        <span>This doubt has been resolved!</span>
                    </div>
                @endif
                ...
                <div class="sp-ticket-footer">
                    ...
                    <span class="sp-ticket-footer-time">
                        <i class="fa-regular fa-clock" style="margin-right:3px;"></i>{{ $ticket->created_at->diffForHumans() }}
                    </span>
                    @if($ticket->messages?->count() > 0)
                        <span style="font-size:0.65rem;color:#007AFF;font-weight:600;">
                            <i class="fa-solid fa-comments" style="margin-right:3px;"></i>{{ $ticket->messages->count() }} message{{ $ticket->messages->count() > 1 ? 's' : '' }}
                        </span>
                    @elseif(!$ticket->admin_reply)
                        <span style="margin-left:auto;font-size:0.65rem;color:rgba(60,60,67,0.45);">
                            <i class="fa-solid fa-hourglass" style="margin-right:3px;"></i>Awaiting reply
                        </span>
                    @endif
                </div>
            </div>
...
function userSendMessage(ticketId) {
    ...
    .then(data => {
        if (data.success) {
            textarea.value = '';
            appendUserBubble(ticketId, data.message);
        }
    })
    .catch(() => {})
...
/* ── Append bubbles ────────────────────── */
function ensureChatLog(ticketId) {
    let log = document.getElementById('sp-log-' + ticketId);
    if (!log) {
        const ticket   = document.getElementById('sp-ticket-' + ticketId);
        const replyBar = document.getElementById('sp-reply-bar-' + ticketId);
        log = document.createElement('div');
        log.className = 'sp-chat-log';
        log.id = 'sp-log-' + ticketId;
        if (replyBar) ticket.insertBefore(log, replyBar);
        else ticket.appendChild(log);
    }
    return log;
}
function appendUserBubble(ticketId, msg) {
    const log = ensureChatLog(ticketId);
    const w = document.createElement('div');
    w.className = 'sp-chat-user-wrap sp-chat-msg sp-chat-bubble-new';
    w.dataset.msgId = msg.id || 0;
...
function appendAdminBubble(ticketId, msg) {
    const log = ensureChatLog(ticketId);
    const w = document.createElement('div');
    w.className = 'sp-chat-admin-wrap sp-chat-msg sp-chat-bubble-new';
    w.dataset.msgId = msg.id || 0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setInterval(() => {
        // Determine which tickets to poll — poll all unsolved tickets
            ...
            .then(data => {
                ...
                if (typeof data.chat_enabled_global !== 'undefined') {
                    ...
                    if (!wasEnabled && CHAT_ENABLED_GLOBAL) {
                        // Chat just got enabled — reload to show reply bars
                ...
                const knownId = lastMsgIds[tId] || 0;
                data.messages.forEach(msg => {
                    if (msg.id > knownId) {
                        const existing = document.querySelector(`#sp-log-${tId} [data-msg-id="${msg.id}"]`);
                        if (!existing) {
                            if (msg.sender_role === 'admin') {
                                ...
                                if (card) {
                                    ...
                                    card.style.boxShadow  = '0 0 0 3px rgba(0,122,255,0.12)';
                                    setTimeout(() => { card.style.borderColor=''; card.style.boxShadow=''; }, 3000);
                                }
                            } else {
                                appendUserBubble(tId, msg);
                            }
                        }
...
document.addEventListener('DOMContentLoaded', startSupportPolling);
</script>
@endsection
